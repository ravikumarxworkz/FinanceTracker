...
def profile(request):
    if request.user.is_authenticated:
        ...
        total_income = Income.objects.filter(user=request.user.id).aggregate(total=django_models.Sum('amount'))['total'] or 0
        
        # Calculate total expenses
        total_expenses = Expense.objects.filter(user=request.user.id).aggregate(total=django_models.Sum('amount'))['total'] or 0
